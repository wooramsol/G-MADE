...
반환 JSON 스키마:
- documentSections는 **아래 평가항목 후보 각각에 대해 1개씩** 작성하라. label은 반드시 해당 평가항목의 detailItem(세부항목명)과 동일하게 쓴다. itemId도 함께 적는다.
- documentSections는 업로드 자료에서 해당 평가항목과 관련해 실제로 읽은 위치만 작성하라. **각 label마다 summary 내용이 서로 달라야 한다.** 모든 항목에 동일한 문장을 반복하지 마라.
- documentSections의 summary는 **평가·판단·보완 요청을 절대 쓰지 말고**, 해당 평가항목을 이해하기 위해 **읽은 파일·페이지·도면·섹션 위치만** 나열한다. 형식: 「파일명」 p.N 도면명·목차·섹션명 — (해당 위치에서 확인한 사실 1줄, 인용 가능). 여러 위치는 줄바꿈으로 1. 2. 3. 목록. "미기재", "불명확", "보완 필요", "심사위원 재확인", "미흡", "저촉" 등 평가·검토 표현 금지.
- documentSections에는 PDF에서 추출된 **깨진 글자·자간이 벌어진 OCR 텍스트(예: "반 영 미")·● 기호·--- 구분선**을 그대로 붙여 넣지 말고, 읽기 쉬운 위치 목록만 작성하라.

documentSections 작성 예시 (위치 목록만, 평가 없음):
"1. 「심의도서.pdf」 p.11 배치도 — 주변 건물 높이·매스 배치 확인\n2. 「심의도서.pdf」 p.12 조감도 — 스카이라인 조망축"
...
- evaluationPreview의 guidelineRefs에는 해당 항목에 실제로 참고한 행정규칙·지침만 적어라. 위 지침 목록에 없는 항목은 인용하지 마라.
{
  ...
  "documentSections": [{ "itemId": "item-urban-scale", "label": "건축물 스케일 적정성", "confidence": 0-100, "summary": "「파일명」 p.N 섹션명 — 확인한 사실 (평가·판단 없이 읽은 위치 목록)" }],
  "evaluationPreview": [{
    "itemName": "평가항목명",
    "score": 0-100,
