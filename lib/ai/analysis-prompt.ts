...
- documentSections의 summary는 해당 도면·계획 유형에서 **확인된 내용 1~2문장**과 **그 유형에서 누락·불명확·추가 검토 필요 사항**만 적는다. 가능하면 「파일명」 p.N을 포함한다. 칭찬·긍정 평가 금지.
...
- 근거 자료(「파일명」 p.N 또는 N면, 도면명, 본문 인용), 위치·공간, 문제, 보완·검토 조치를 반드시 포함.
...
- 구조: [근거 자료·위치] → [구체적 문제 2~5개를 1. 2. 3. 형식으로 각 줄에 하나씩 나열] → [실행 가능한 보완·검토 조치].
...
"「사업계획서.pdf」 p.3 배치도·보행동선도 기준, 증축으로 확보되는 2층 옥외공간 관련하여 다음 사항의 수정·보완·재확인이 필요합니다.\n1. 고령 이용자 휴게·이동 동선상 계단 단 높이·경사 및 미끄럼 방지 바닥재 기준이 p.3 배치도·계획서 본문에 수치로 제시되지 않음\n2. 옥외 휴게 구간 난간 높이·차양(계단에서 출입구 이동) 설치 위치가 p.4 입면도에 표기되지 않음\n3. 해당 구간 조도·그늘 확보 방안이 p.6 야간경관·조경 계획과 연계되어 있지 않아 실시설계 단계에서 상호 검토가 필요함\n\n위 항목을 해당 페이지 도면에 수치·재료·시공 상세와 함께 구체화 하시기 바랍니다."
