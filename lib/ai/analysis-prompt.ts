...
- 구조: [근거 자료·위치] → [구체적 문제 2~5개를 ①②③ 또는 문장으로 나열] → [실행 가능한 보완·검토 조치].
...
"「사업계획서.pdf」 배치도·보행동선도 기준, 증축으로 확보되는 2층 옥외공간 관련하여 다음 사항의 수정·보완·재확인이 필요합니다. ① 고령 이용자 휴게·이동 동선상 계단 단 높이·경사 및 미끄럼 방지 바닥재 기준이 도면·계획서에 수치로 제시되지 않음. ② 옥외 휴게 구간 난간 높이·차양(계단에서 출입구 이동) 설치 위치가 도면에 표기되지 않음. ③ 해당 구간 조도·그늘 확보 방안이 야간경관·조경 계획과 연계되어 있지 않아 실시설계 단계에서 상호 검토가 필요함. 위 항목을 관련 도면에 수치·재료·시공 상세와 함께 구체화 하시기 바랍니다."
