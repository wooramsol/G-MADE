...
/** 파일 1개당 AI 프롬프트에 넣을 최대 글자 수 (기존 40,000의 2배) */
export const PER_FILE_AI_TEXT_CHARS = 80_000;
/** OpenAI용 합산 상한 (기존 80,000의 2배) */
export const TOTAL_AI_TEXT_CHARS = 160_000;
/** Gemini·Claude용 합산 상한 (기존 45,000의 2배) */
export const LIGHTWEIGHT_PROVIDER_TOTAL_AI_TEXT_CHARS = 90_000;

const SNIPPET_RADIUS = 1_200;
const MAX_SNIPPETS = 10;
const HEAD_SNIPPET_CHAR_CAP = 36_000;
...
export function trimTextForAiAnalysis(text: string, charBudget: number): TextBudgetResult {
  ...
  const headBudget = Math.min(Math.floor(usable * 0.45), HEAD_SNIPPET_CHAR_CAP);
