...
치수·수치 인용 규칙(오류 방지 최우선): 확대본에서 숫자와 단위가 "또렷하게" 읽히는 경우에만 인용하고, 그 note는 반드시 "확대 판독: "으로 시작하며 읽은 수치를 원문 그대로(단위 포함) 기재하세요 (예: "확대 판독: 옹벽 H=1.8M 표기 확인"). 한 글자라도 흐릿하거나 잘려 있으면 그 수치는 인용하지 말고 다른 근거만 쓰세요. 어림·계산·환산한 수치는 절대 금지합니다.
