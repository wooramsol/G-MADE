...
type TrashedRoundsPanelProps = {
  ...
  onPurged?: (activeRounds: EvaluationRound[], trashedRounds: EvaluationRound[]) => void;
};

export default function TrashedRoundsPanel({
  project,
  trashedRounds,
  onRestored,
  onPurged,
}: TrashedRoundsPanelProps) {
  const [restoringRoundId, setRestoringRoundId] = useState<string | null>(null);
  const [purgingRoundId, setPurgingRoundId] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  if (trashedRounds.length === 0) return null;

  const restoringRound = trashedRounds.find((round) => round.id === restoringRoundId);
  const purgingRound = trashedRounds.find((round) => round.id === purgingRoundId);
  ...
  async function purgeRound() {
    if (!purgingRoundId) return;

    const roundId = purgingRoundId;
    setLoading(true);
    ...
    try {
      ...
      onPurged?.(activeRounds, nextTrashed);
      setPurgingRoundId(null);
      showToast({ message: "평가 기록이 영구 삭제되었습니다.", tone: "success" });
    } catch (error) {
      showToast({
        message: error instanceof Error ? error.message : "평가 기록 영구 삭제에 실패했습니다.",
        tone: "error",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <section className="rounded-2xl border border-dashed border-[#d7dee8] bg-[#f8fafc] p-5">
      ...
      <Caption className="mt-1 text-[#64748b]">
        삭제한 평가 {trashedRounds.length}건이 보관되어 있습니다. 복원하거나 영구 삭제할 수 있습니다.
      </Caption>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                type="button"
                onClick={() => setPurgingRoundId(round.id)}
              >
                영구 삭제
              </button>
            </div>
      ...
      <ConfirmDialog
        ...
        description={
          purgingRound
            ? `${formatEvaluationRoundLabel(purgingRound.evaluatedAt)} 평가를 영구 삭제합니다. 되돌릴 수 없습니다.`
            : "선택한 평가 기록을 영구 삭제합니다. 되돌릴 수 없습니다."
        }
        loading={loading}
        open={Boolean(purgingRoundId)}
        title="평가 기록 영구 삭제"
        onCancel={() => {
          if (!loading) setPurgingRoundId(null);
        }}
        onConfirm={purgeRound}
      />
    </section>
  );
}
